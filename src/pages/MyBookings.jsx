...
export default function MyBookings({ onNavigate }) {
  ...
  const getStatusBadge = (status, statusDetail) => {
    switch (status) {
      case 'Confirmed':
        return (
          <div className="bg-[#E6FBF2] border border-[#BFF3DB] rounded-xl px-4 py-2 w-full text-center">
            <div className="flex items-center justify-center gap-1.5 text-[#0F766E] font-bold text-[11px] uppercase tracking-wider">
      ...
      case 'Pending Confirmation':
        return (
          <div className="bg-[#FFF8E6] border border-[#FFE4A3] rounded-xl px-4 py-2 w-full text-center">
            <div className="flex items-center justify-center gap-1.5 text-[#D97706] font-bold text-[11px] uppercase tracking-wider">
      ...
      case 'Completed':
        return (
          <div className="bg-[#F1F5F9] border border-[#E2E8F0] rounded-xl px-4 py-2 w-full text-center">
            <div className="flex items-center justify-center gap-1.5 text-slate-600 font-bold text-[11px] uppercase tracking-wider">
      ...
      case 'Cancelled':
        return (
          <div className="bg-[#FEF2F2] border border-[#FEE2E2] rounded-xl px-4 py-2 w-full text-center">
            <div className="flex items-center justify-center gap-1.5 text-red-650 font-bold text-[11px] uppercase tracking-wider">
                ...
                <div className="flex flex-col gap-5">
                  {filteredBookings.length > 0 ? (
                    filteredBookings.map((booking) => (
                      <div 
                        key={booking.id}
                        className="bg-white rounded-3xl border border-slate-200 p-5 flex flex-col xl:flex-row gap-6 shadow-sm transition-all duration-300 hover:shadow-md hover:border-slate-300 text-left group animate-fade-in"
                      >
                        {/* Booking Image */}
                        <div className="w-full xl:w-[240px] h-[165px] shrink-0 rounded-2xl overflow-hidden bg-slate-100 relative">
                          <img 
                            src={booking.image} 
                        ...
                        <div className="flex-grow flex flex-col justify-between gap-4 min-w-0">
                          ...
                          {/* Top Section: Side-by-side Details & Status/Price */}
                          <div className="flex flex-col md:flex-row justify-between items-stretch gap-6">
                            
                            {/* Details Column */}
                            <div className="flex-grow space-y-2 min-w-0">
                              <h3 className="text-xl font-extrabold text-slate-900 tracking-tight leading-snug hover:text-[#0F766E] transition-colors cursor-pointer">
                                {booking.hotelName}
                              </h3>
                              
                              {/* Location Line */}
                              <div className="flex items-center gap-1.5 text-xs text-[#0F766E] font-bold">
                                <MapPin className="h-4 w-4 text-[#0F766E] shrink-0" strokeWidth={2.2} />
                                <span className="text-slate-600 font-semibold">{booking.location}</span>
                            ...
                            <div className="w-full md:w-[220px] shrink-0 border-t md:border-t-0 md:border-l border-slate-200 pt-4 md:pt-1 pl-0 md:pl-6 text-left flex flex-col justify-between gap-4">
                              ...
                              <div className="w-full text-left flex flex-col justify-center">
                                ...
                                <span className="text-2xl font-black text-slate-900 mt-1.5 block leading-none">{booking.amount}</span>
                                <span className={`text-[10px] font-bold mt-1.5 block leading-none ${
                          ...
                          {/* Bottom Section: Booking ID & Date (Left) & Buttons (Right) */}
                          <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 pt-4 mt-2">
                            {/* Booking ID & Date Block */}
                            <div className="flex flex-col gap-0.5 text-left">
                              <div className="text-[13px] leading-tight">
                                <span className="text-slate-900 font-bold">Booking ID: </span>
                                <span className="text-slate-900 font-bold">{booking.id}</span>
                              </div>
                              <div className="text-xs text-slate-500 font-semibold leading-tight mt-0.5">
                                <span>Booked on: </span>
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                              <button 
                                ...
                                className="border border-[#0F766E] text-[#0F766E] hover:bg-teal-50 font-bold text-xs px-5 py-2.5 rounded-xl transition-all cursor-pointer whitespace-nowrap"
                              >
                                View Details
                              </button>
                              ...
                              {booking.status === 'Pending Confirmation' && (
                                <button 
                                  ...
                                  className="border border-red-200 text-red-500 hover:bg-red-50 font-bold text-xs px-5 py-2.5 rounded-xl transition-all cursor-pointer whitespace-nowrap"
                                >
                                  Cancel Booking
                                </button>
                              )}
                              ...
                              {booking.status === 'Confirmed' && (
                                <button 
                                  ...
                                  className="border border-[#0F766E] text-[#0F766E] hover:bg-teal-50 text-xs px-5 py-2.5 rounded-xl transition-all cursor-pointer font-bold flex items-center gap-1.5 whitespace-nowrap"
                                >
                                  <Phone className="h-3.5 w-3.5 text-[#0F766E]" strokeWidth={2.3} />
                              ...
                              {booking.status === 'Completed' && (
                                <button 
                                  ...
                                  className="border border-[#0F766E] text-[#0F766E] hover:bg-teal-50 text-xs px-5 py-2.5 rounded-xl transition-all cursor-pointer font-bold flex items-center gap-1.5 whitespace-nowrap"
                                >
                                  <Star className="h-3.5 w-3.5 text-[#0F766E]" strokeWidth={2.3} />
